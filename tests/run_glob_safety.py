...
#   🔴 [2026-08-01 3차 · 등급 분리] 셋을 같은 등급으로 묶지 않는다.
#     **표시 오염과 판정 오염은 심각도가 다르다.** 판정 계열은 KNOWN 에 두지 않는다 — 즉시 차단한다.
#     ⚠ 실측으로 등급이 **뒤집혔다**: `_snapshot_metas_for_race` 를 '표시'로 봤는데
#       `_snapshot_build_compare` 가 여기서 나온 단계별 메타로 **변화량을 계산해 저장**하고
#       그 결과가 `snapshot_timing.json`(T-10 vs T-2 적중률 학습)에 누적된다.
#       compare 114개 중 **8개(7.0%)가 실제로 다른 날짜를 짝지었다**(예: `나고야 11경주` —
#       T-10·T-2 는 07-29, 마감후는 **07-15** → "최저배당 변화 -56.7%" 가 그 짝짓기에서 나온 값이다).
#       ⇒ **KNOWN 에서 제거. 즉시 차단 대상.**
KNOWN = {
    # 🟡 B급(판정 인접 · 실오염 0건) — 아래 4가지 근거로 통계는 안전하다.
    #   ① 대량 경로 `_high_odds_scan` 은 `rr` 를 직접 넘겨 이 함수를 **타지 않는다**
    #   ② 유일한 호출부 `/api/high-odds/review` 는 현재 로그 구간 호출 **0건**
    #   ③ 저장 파일명이 `<date>_<rk>.json` **날짜 접두**라 다른 날 리뷰를 덮지 않는다
    #   ④ 로드 후 `rk`·`date` 를 문서에서 다시 읽어 **내부 정합은 유지**된다
    #   ⚠ 그래도 위험 자체는 크다 — `race_results` 1,880건 중 **349종이 2개 이상 파일에 매칭**된다.
    ("app.py", "_rr_load_by_key"):
        "🟡 B급 판정인접 · 실오염 0건(대량경로 rr 직접전달·호출 0건·날짜접두 저장) · 349종 다중매칭 위험",
    # 🟢 C급(순수 표시) — 카드·목록의 Gemini 뱃지. 계산·저장·학습 어디에도 안 들어간다.
    ("app.py", "_gemini_latest_map"):
        "🟢 C급 표시전용 · {raceKey: 리뷰} 카드 뱃지 · 판정/학습 미접촉",
}

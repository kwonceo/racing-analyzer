...
# ── [3번] 확률/기대값 _prob_ev(o, placings) [배당 우선 전환 70/30] ──────────
#   o=4(≤5배 → 시장 유력), 최근5 착순 [1,2,3,4,5] → 입상 3회 → 전적 0.6, ×1.1 보정 = 0.66
#   시장 = 1/4*0.75 = 0.1875 → 18.8% / 통합 = .1875*.7 + .66*.3 = .32925 → 32.9%
#   기대값 = .32925*4 - 1 = 0.317 → +31.7%
print("[3번] 확률/기대값 [배당 우선] (o=4, 착순[1,2,3,4,5])")
mkt, form, comb, ev = app._prob_ev(4, [1, 2, 3, 4, 5])
chk("시장확률 = 1/배당×0.75 ≈ 18.8%", approx(mkt, 18.8), str(mkt))
chk("전적확률 = 입상3/5 ×1.1(5배↓ 보정) = 66%", approx(form, 66.0), str(form))
chk("통합확률 = 시장×0.7+전적×0.3 ≈ 32.9%", approx(comb, 32.9), str(comb))
chk("기대값 = 통합×배당-1 ≈ +31.7%", approx(ev, 31.7, 0.5), str(ev))
# [배당 우선] 고배당(10배+) → 전적 × 0.7 감가: o=20, 입상 0.6 → 0.42
mkt3, form3, comb3, ev3 = app._prob_ev(20, [1, 2, 3, 4, 5])
chk("고배당(20배) 전적확률 = 0.6×0.7 = 42%", approx(form3, 42.0), str(form3))
# 배당 미수집 시 방어

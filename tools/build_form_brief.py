...
5-C. 🔴🔴 **다음은 빼는 근거로 쓰지 않는다(감점 금지 목록 · 대표 지시 2026-08-10).**
   무승(1착 0회) / 무입상 / 첫 출주(신마·데뷔) / 나이가 많다·어리다 / 표본이 얇다(전적 수가 적다)
   ⚠ **이미 배당에 반영돼 있다.** 시장이 아는 것을 우리가 다시 깎으면 두 번 깎는 것이다.
   ⚠ 2026-08-10 하루에 이 유형으로 **일곱 번 틀렸다.**
   🟢 사실로 적는 것은 된다: "1착이 없습니다" 는 사실이다.
     🔴 «1착이 없어서 뺍니다» «표본이 얇아 신뢰하기 어렵습니다» 처럼 **판단 근거로 쓰는 것**을 금한다.
   ⚠ A-7 과 정합한다 — «1착 0회 · 2착 다수»는 오히려 **가점** 대상이다.
6-A. 🔴🔴 **말 이름을 부마(父)와 혼동하지 않는다(2026-08-10 실사고).**
   oddspark 출주표는 한 말에 가타카나가 여러 줄 나온다. 순서는 이렇다:
     첫 줄 = **부마(父)** · 둘째 줄 = 🔴 **말 이름** · 그다음 «모마(모부)» 순.
   ⚠ 실제로 1번을 «ニホンイチ»(부마)로 적었는데 말 이름은 «ビックドラゴン» 이었다.
   🔴 어느 줄이 말 이름인지 확신이 서지 않으면 **마번만 쓴다**("1번"). 이름을 지어내지 않는다.
...
A-5. 🔴🔴 **경고만 적고 그대로 내지 않는다(대표 지시 2026-08-09).**
     ⑦ 주의점에 «N번이 위험하다»고 적었으면 **그 말을 ⑥ 조합에 반드시 넣는다.**
     넣기 싫으면 ⑦에서 그 말을 빼라. 둘 중 하나다. 어기면 그 분석문은 폐기된다.
     ⚠ 조합으로 못 막는 위험(표본 얇음·배당 없음·휴양 공백)은 예외다 — 마번을 쓰지 않으면 된다.

A-6. 🔴 **라인 구성원을 개별 성적으로 쪼개지 않는다(경륜).**
     라인 자리(선두·번수·3번수)가 개별 성적보다 강하다.
     ⚠ 8/7 에 여섯 번 쪼개서 **여섯 번 다 틀렸다.**
     라인 독점을 예상하면 **그 라인 안의 짝도 복승에 넣는다.**

A-7. 🟢 «1착 0회 · 2착 다수»인 말은 복승·삼복승에 **가점**한다(8/7 실전 4전 4승).
     ⚠ 반대로 **«3착 이내 비율»은 축으로 쓰지 않는다**(1위가 7전 1승).
     ⚠ **«선두가 약하면 라인이 무너진다»도 쓰지 않는다**(2전 2패).

A-8. 축이 확실하면 축에 얹고, 애매하면 **삼각형으로 묶는다.**

A-9. 🔴🔴 **반에이(ばんえい·오비히로)는 아래 셋을 순위로 세운다(대표 방식 2026-08-10).**
     실물: 2026-08-10 오비히로 2R **55.3배**가 이 두 축만으로 나왔다 —
     «부담 대비 마체중 1위» + «마체중 증가폭 최대».
     ① **부담 대비 마체중** — 원문의 «마체중»과 «부담중량»을 **그 말 옆에 그대로 병기**한다.
        🔴🔴 **순위(1위·2위)를 매기지 않는다.** 나눗셈도 하지 않는다.
        ⚠ 2026-08-10 에 순위를 매기게 했더니 **정반대로 나왔다**(1.51 을 1위로, 실제 1위 1.76 을 누락).
          LLM 이 나눗셈을 못 해서가 아니라 **원문에서 부담중량을 못 읽는 말이 섞이기 때문**이다.
          숫자를 병기만 하면 회원이 직접 비교할 수 있다. **틀린 순위보다 병기가 낫다.**
        🟢 이렇게 쓴다: "8번 마체중 829kg · 부담중량 470kg" (여기서 멈춘다)
        🔴🔴 **비율값(1.51 · 1.76 같은 소수)을 문장에 절대 쓰지 않는다.**
          그 숫자는 원문에 없으므로 검증기가 «원문에 없는 숫자»로 잡아 **그 분석문 전체가 폐기된다.**
          ⚠ 실제로 두 번 연속 폐기됐다(2026-08-10). «부담 대비» 라는 말만 쓰고 숫자는 원문 값 둘만 적는다.
     ② **마체중 증감폭** — «+15kg» 처럼 원문 표기 그대로. 🔴 **늘어난 쪽을 위로** 본다.
        🟢 이 축만 원문에 부호까지 적혀 있어 **비교해도 된다**(2026-08-10 실측에서 유일하게 맞았다).
        ⚠ 줄어든 것을 빼는 근거로 쓰지 않는다(규칙 5).
     ③ **착차(1착마와의 차이)** — 원문에 있으면 그 값을 병기한다.
        🔴 **최소 순위를 매기지 않는다.** ①과 같은 이유로 2026-08-10 에 틀렸다.
     🔴 ②에서 증가폭이 가장 큰 말은 ④에서 반드시 다루고 ⑥ 조합에 넣는다.
     ⚠ 원문에 마체중·부담중량이 없으면 «원문에 없다»고 적고 넘어간다.
     ⚠ `axisRanks` 같은 **순위 목록 항목을 만들지 않는다.** 값은 ④ 말별 설명 안에 병기한다.

A-10. 🔴 **경륜 라인 짝은 빠짐없이 넣는다(대표 방식 2026-08-10).**
     ① 라인 독점(그 라인이 상위를 쓸어담는 그림)을 **삼복승에 넣었으면**
        🔴 **그 라인 안의 복승 짝 셋을 전부 ⑥ 조합에 넣는다.** 하나만 넣고 끝내지 않는다.
        (라인 A-B-C 면 A+B · A+C · B+C 셋이다.)
     ② 상대로 넣은 말이 다른 라인이면 🔴 **그 말의 번수(2번수)도 짝으로 넣는다.**
        선두만 쓰고 번수를 버리면 라인이 그대로 들어왔을 때 못 잡는다.
     ⚠ 라인 구성이 원문에 없으면 억지로 만들지 않는다.
...
A-11. 🔴🔴 **`summary` 는 여섯 줄이다. 지금 길이는 아무도 안 읽는다(대표 지적).**
     ...
     🔴 **사실 나열이 아니라 「왜 골랐나」를 쓴다(대표 지시 2026-08-09).**
        숫자를 먼저 던지고 **줄표 뒤에 이유 한 줄**을 붙인다. **두 줄을 넘기지 않는다.**
        ❌ "1:01.5로 가장 빠르며 직전 3착을 기록했습니다"   ← 사실 나열
        🟢 "1:01.5 — 1000m 를 가장 빠르게 뛴 말입니다"
        🟢 복병은 **왜 시장이 안 보는지**를 쓴다:
           "능력검사에서 누구보다 빨랐는데 시장은 안 봅니다"
     🔴 **조교·기수를 근거로 쓴다(PDF 에 있는 것만).**
        조교 훈련시간 증감(직전 2주 ↔ 이번 2주) · 기수 복승률 3개월·6개월.
        🟢 둘이 같은 방향이면 강조: "조교 111분 최다 · 기수 복승률 32%% 1위"
        ⚠ **없는 궁합을 지어내지 않는다** — "이 기수가 이 말을 탔을 때"는 데이터가 없다.
     🔴 **마명이 깨져 보이면 마명을 쓰지 말고 마번만 쓴다.**
        원문 PDF 에 **글자 단위로 깨진 것**이 섞여 있다(부산 1R 18자 · 2R 30자 실측).
        한글로 읽히지 않는 글자가 하나라도 있으면 그 이름은 **통째로 생략**하고 "8번" 으로만 쓴다.
        ⚠ 깨진 이름을 내보내는 것보다 낫다. **마번은 정확하다.**
...
F-2. 🔴🔴 **축 하나에 모든 조합을 묶지 않는다.**
   조합을 3개 낸다면 같은 말을 세 번 다 넣지 않는다.
   🔴 **저평가 후보가 둘 이상이면 후보들끼리 묶은 조합을 반드시 하나 이상** 만든다.
   ⚠ 실제 사고(2026-08-06): 몬베츠 9R 조합이 9+7 · 9+11 · 7+8 로 9번에 몰렸고
     정답 11+7 이 없었다. 후나바시 9R 도 5번에 몰려 정답 3+4 가 없었다.
     축을 낀 조합만 만들면 **후보끼리의 조합이 영영 안 나온다.**
   권장 배분: 조합 3개면 축을 낀 것 2개 + **후보끼리 1개**.
...
def verify(doc, body, valid_nos):
    ...
    #   ⚠ 8/7 실전에서 이 유형으로 **여섯 번 틀렸고 여섯 번 다 그 말이 3착권**에 들었다.
    #   ⇒ ⑦ 주의점에 **마번이 이름으로 나왔는데 ⑥ 조합에 그 말이 하나도 없으면** 폐기한다.
    ...
    try:
        ...
        if _warn_nos:
            ...
            for c in (doc.get("combos") or doc.get("quinellas") or []):
                for x in (c.get("combo") if isinstance(c, dict) else c) or []:
                    try:
                        _in_combo.add(int(x))
                    except Exception:
                        pass
            _miss = sorted(_warn_nos - _in_combo)
            if _miss and _in_combo:
                problems.append("⑦ 주의점에 %s번을 적어놓고 ⑥ 조합에 안 넣었다(경고만 하고 그대로 냄)"
                                % "·".join(str(x) for x in _miss))
    except Exception:
        pass
    ...
    if und:
        ...
        # 🔴 [2026-08-06 대표 지시 ④] 축 하나에 모든 조합을 묶지 않는다.
        #   후보가 2두 이상인데 **후보끼리 묶은 조합이 하나도 없으면** 지적한다.
        #   ⚠ 후보 1두면 구조적으로 불가능하므로 면제.
        #   ⚠ 지금은 **경고**다 — 발동률을 먼저 재고 폐기로 올릴지 정한다(원칙 4·20).
        if len(und) >= 2:
            pair = any(len(set(int(x) for x in (c.get("combo") or [])) & set(und)) >= 2
                       for c in (doc.get("combos") or []))
            if not pair:
                problems.append("⚠경고(폐기 아님) 저평가 후보끼리 묶은 조합이 없다 %s" % und)
...
def market_table(html, kind):
    ...
    for h in sorted(hs, key=lambda x: x["no"]):
        ...
        #   🔴 [2026-08-06 재조정] '승수 0' 으로 잡으면 **너무 넓다** — 몬베츠 5R 8번은
        #     距 (0,0,1,2) = 3전에 3착 1회인데 통째로 빠졌고 그 말이 2착이었다.
        #     후나바시 11R 11번은 距 (0,0,0,3) = **완전 무입상**이다. 둘은 다르다.
        #   ⇒ 필터를 **당거리 무입상**(1·2·3착이 전부 0)으로 좁힌다. 두 실증을 다 만족한다.
        #   ⚠ 사후 조정이다. 표본이 쌓이면 배수로 재검증한다.
        _d = h.get("dist")
        dist_zero = bool(_d) and sum(_d) > 0 and (_d[0] + _d[1] + _d[2]) == 0
    ...
    # 🔴 [2026-08-06 대표 지시] **절대 임계를 버리고 그 경주 안 상대 순위**로 바꾼다.
    #   두수가 다르면 괴리 범위가 다르다 — 14두는 +12 까지 나오고 7두는 +6 이 최대다.
    #   실증 다섯으로 맞춘 최소값이 **N=3** 이다(몬베츠 4R 2위 · 9R 1위 · 12R 3위 · 후나바시 9R 3위).
    #   ⚠ N 을 실증에 맞춘 것은 **사후 최적화**다. 표본이 쌓이면 배수로 재검증하고,
    #     N 을 키우는 방향으로는 쉽게 바꾸지 않는다.
    TOP_N = 3
    GAP_MIN = -99
    ...
    for g, n, nm, mk in gaps:                 # ⚠ 같은 말이 괴리·2착형으로 두 번 들어올 수 있다
        ...
        if g != 99 and len([x for x in top if x[0] != 99]) >= TOP_N:
            break                          # 괴리 상위 N두까지. 2착형(99)은 별도 축이라 세지 않는다
    tail = ""
    if top:
        ...
        try:
            for _p in probs:
                if "심리" in _p:
                    ...
                    _bump("d_summary")   # 🔴 [2026-08-09] 종전엔 d_other 로 묻혔다

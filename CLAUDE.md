...
- **할 일**: 조합 수 급감(직전 대비 70% 미만)을 **단독 게이트**로 추가 — 배당은 저장하되 기준값 갱신·급락 계산에서 제외.
...
- **할 일**: ⓐ서버·확장 양쪽 파서의 열 정합 검증(같은 경주를 두 경로로 받아 대조)
...
### 원인 — ⓐ EV 필터의 고배당 암살 (가장 큰 단일 원인)
- 강등 사유 1위: `기대값 N 미달(배당 N배 × 추정적중률 N%)` — **526건**(2위 37건과 격차 큼).
- 실제 로그: `[8,10] 20.5배 × 추정적중률 **1%** → 기대값 0.28 → 강등`
  그러나 **20~50배 실측 적중률은 3.2%** → **3배 과소평가**. 고배당이 기계적으로 잘려나간다.
- **할 일 1**: 배당대별 실측 적중률(위 표)로 추정적중률 산출식 재보정 → **리플레이로 회수율·고배당 비율 검증**.
  (선택·정렬보다 위험이 작다. 오늘 진성급락 정렬 시도는 회수율 100.2%→91.4%로 악화해 기각됐다.)
...
- ❌ **말별 각질(추입·선행) 데이터는 전무**: `style`/`runningStyle`/`position` **0건**.
  부스팅할 대상 자체가 없어 로직을 올려도 공회전한다(`signalQuality`·단승 수집과 같은 실패 유형).
...
- **할 일 2**: 각질 수집·저장(NAR 출마표 `DebaTable` / 경륜 선수 각질) →
  **할 일 3**: 「빠른 페이스 × 추입마」 조합의 실제 적중률·배당 **리플레이 검증**(페이스 242건은 이미 있으니 각질만 채우면 즉시 가능) →
  **할 일 4**: 검증 통과 시 `_scenario_combos` 부스팅 활성화 + 가짜 인기마(인기1위가 선행인데 하이페이스) 페널티.
  ⚠ **3번을 건너뛰지 말 것** — 오늘 그럴듯한 가설 3개가 리플레이에서 전부 뒤집혔다.

### 우선순위 요약
1. **EV 필터 재보정**(할 일 1) — 영향 526건, 근거 데이터 확보됨, 위험 작음
2. 각질 수집(할 일 2) → 전개 리플레이(할 일 3) → 부스팅(할 일 4)

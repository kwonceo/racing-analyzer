...
## 🔴🔴 다음 세션 우선순위 (2026-08-01 세션 종료 시점)

1. **경주기록 스냅샷 날짜 오염** — 🔴 **실사고 · 회원 화면에 보인다.**
   7/31 클릭 → 7/14·7/30 스냅샷 반환. `raceKey` 에 날짜가 없어서다. **미착수.**
2. **하루 마감 시각 확인** — 🔴 **22시 vs 23:30 충돌.**
   대표 확인은 "저녁 10시면 끝"인데 시스템은 마지막 발주를 **23:30** 으로 읽는다.
3. **`_GRADE_LOCK` 파일화** — 메모리→파일(D) 항목 1위. 마감 동결과 직결. **설계안부터.**
...
## 🔴 [2026-08-01] 하루 마감 시각 — **대표 확인과 시스템이 다르다 (미해결)**

| | |
|---|---|
| 🔴 **대표 확인** | **하루 마감은 저녁 10시다.** |
| 🔴 시스템 실측 | 7/31 마지막 발주 **23:30**(다케오 7R · 15틱 정상 수집) |

- 실측으로는 21시 이후 **18건**이 전부 **미드나이트 경륜**(코치·케이오카쿠·다케오)이었고
  **스케줄 파싱 오류가 아니었다**(위 별도 항목의 전수 표 참조).
- 🔴 **그럼에도 대표 확인이 "10시"이므로 이 충돌은 미해결이다.**
  내가 "미드나이트 경륜이니 23:30 이 맞다"고 결론 낸 것은 **성급했다** —
  대표가 실제로 운영하는 하루의 끝이 기준이어야 한다.
- **다음 세션 확인 대상**:
  · 미드나이트 경륜을 우리가 **추천 대상으로 삼고 있는가**(카톡을 보내는가)
  · 안 보낸다면 **분모에서 빼야** 한다 → `_is_settled()` 기준도 22:40 이 된다
  · 보낸다면 대표가 그 시간대를 **안 보는 것**이므로 그것대로 별도 판단이 필요하다
- ⚠ 이 값이 바뀌면 **I1·I4 확정 시각**과 **카카오 발송 시각**이 함께 바뀐다.

...
### 📏 세션 규칙
1. 🔴 **새로 발견한 것을 그 자리에서 쫓지 말 것. 발견은 목록에만 남긴다.**
   (오늘 하루 발견이 계속 새 작업을 낳아 크리티컬 패스가 밀린 것에 대한 규칙)
2. 🔴 **"이게 더 급하다"는 판단을 세션 중에 하지 않는다 — 다음 세션에서 정한다.**
3. **세션 종료 보고**: 발견 목록 + **다음 목표 제안 3줄** + **체크리스트 N/19**.
4. ⚠ **[원문 유실 · 다음 세션 확인]** 지시 원문에 `정의` / `단계는 끝이다` 조각이 있었으나
   전송이 잘려 전체 문장을 복원하지 못했다. 문맥상 **"완료 정의 — 이 단계는 끝이다"**
   (단계 종료 조건 명시)로 보이나 **확정하지 않는다.** 다음 세션에서 원문을 받아 채운다.

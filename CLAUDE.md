...
- ✅ **일본경마 페이스 보유율 23.6% — 버그 아님(원인 규명 완료).**
  ...
  **최근 4일 85.4%(140/164) — 목표 70% 이미 초과 달성.** 수집 파이프라인 수정 불필요.
  ...
  - ⚠ **별건 발견 — 경륜장이 '일본경마'로 오분류**: 기후 32 · 마쓰도 24 · 기시와다 24 · 사세보 30건이
    `sport=horse`로 저장돼 종목별 통계를 오염시킨다. **전부 7/22 이전**이고 이후엔 없다
    (`_KEIRIN_ONLY_RE` 지명 보강으로 이미 해소됨) → **소급 정정만 남음**(집계 시 제외 권장).
...
- ⏭ **경마용 `horse_profiles.jsonl`은 미착수** — 설계안은 승인됐으나, `raw_profile`이 실제로 채워지는지
  다음 개최일에 확인한 뒤 생성한다(빈 원본으로 백필하면 경륜과 같은 null 투성이가 된다).

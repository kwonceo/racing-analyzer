...
1. **승인 3건** — ② **테스트 범위 확대(`os.listdir`)** → ① **스냅샷 날짜 수정** → ③ **완료조건 미확정**
   ⚠ 순서 고정. ②의 정규식 3개는 이미 파일에 있으나 `scan()` 미반영 상태다.
2. **`starters_store` 0/54** — 분석 입력이면 판정 오염이다.
   ✅ **작업 0에서 이미 확인함 — 분석 오염 없음**(덮어쓰기 저장 + 정확 일치 조회 + 24h 정리,
   24h 초과 잔존 0개). **남은 것은 구조적 위험뿐**(같은 경주가 연속 이틀이면 24h 창 안에서 어제 것을 읽음).
   → **재조사하지 말 것.** `results_store` 의 `사세보 2경주` 1건만 실질 위험.
3. **미드나이트 ②안 적용** — **측정 분모 전수**. `_is_settled()` 기준 = 미드나이트 제외 마지막 발주 + 40분.

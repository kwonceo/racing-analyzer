...
- **판정: 겹침률 30~40% 로 기준(80%)을 크게 밑돈다 → "새 정보 아님"이 아니다.**
  총평은 시장(전적 상위)과 **다른 대상을 지목**하는 경우가 60~70%다. **배당판 종속 탈피 후보로 유효하다.**
  ⚠ 단 비교 기준이 **전적 총점 1위**다(`starters_store` 에 배당이 없어 시장 1위를 직접 못 씀).
    **배당 기준 재측정이 필요**하며, 그 전까지 이 40%/30%는 잠정치다.
...
- ⏸ **조치안(승인 대기·미구현)**: `triple_analyze()` 엔드포인트에서 `afterClose && readonly` 면
  **저장을 건너뛰고 기존 로그를 그대로 반환**. ⚠ 추천 경로라 임의 수정 금지.

...
# 🔴🔴 [2026-08-03 · 종결 판정] F2·F3 **동시 열위**로 Gemini 독립 예측을 종료한다.
#   판정선(변별 100 · 동률 제외)을 **낮추지 않고** 도달한 뒤 판정했다.
#     F2  Gemini 평균 **1.73** ↔ 시장 **1.94** · 변별 102(우위 31 · 동률 134 · 열위 71)
#     F3  단독 적중 **57** ↔ 시장 **98** · 가상 회수율 **70.4% ↔ 70.7%**(구좌 동일)
#         상위3 제외 **53.8% ↔ 62.3%** — 극단값을 빼면 격차가 **더 벌어진다**
#   🔴 "덜 맞추는데 크게 맞는다"는 **불성립**이다: 배당중앙 6.2 ↔ 5.4 로 높지만
#     적중이 시장의 **58%** 라 회수율이 동률이다. 경마는 배당중앙마저 7.4 ↔ 7.4 로 같다.
#   🔴 두 지표가 **같은 방향**이라 F2 단독 종결이 아니다(F3 를 나란히 보고 판정했다).
#
# 🟢 **코드·로그는 지우지 않는다.** `logs/forecast/` 예측 기록 전량 보존 — 재개 시 대조 자료다.
#
# 🔧 되살리는 조건 (아래 중 **하나라도** 성립하면 다시 본다):
#   ⓐ 입력이 바뀐다 — `past[]` 시계열·과거 인기(`pop`)가 쌓여 **프롬프트에 없던 정보**를 넣을 수 있게 될 때
#      (2026-08-03 현재 past.pop 보유 0%. 이것이 있었다면 예측 근거 자체가 달랐다)
#   ⓑ 질문을 바꾼다 — top3 예측이 아니라 **"1인기가 무너지는가"**(제외마) 로 전환할 때.
#      표본이 많고 판정이 명확하며, 우리가 가장 약한 ①유형(아예 안 본 말)과 직결된다.
#   ⓒ 시장 기저선이 바뀐다 — `pop_baseline.json` 이 지방·경륜까지 확장돼 대조군이 정확해질 때
#   ⚠ 어느 경우든 **판정선(변별 100 · 동률 제외)은 그대로** 쓴다. 사후에 낮추지 않는다.
FORECAST_TERMINATED = True          # 종결 표시(문서·체크리스트용 · 동작은 아래 플래그가 결정)


def forecast_enabled():
    """[Phase A] 예측 호출 허용 여부. 🔴 **2026-08-03 종결** — 기본 꺼짐이고 종결 플래그가 우선한다.

    ⚠ `GEMINI_REVIEW_ENABLED`(코드 리뷰) 와 **별개**다.
    🔧 재개: `FORECAST_TERMINATED = False` 로 바꾸고 `GEMINI_FORECAST_ENABLED=1` 을 준다.
      (환경변수만으로는 켜지지 않는다 — 종결 판정을 사람이 명시적으로 되돌리게 하기 위함이다)
    """
